...
def can_use_calculator(s: str) -> bool:
    ...
    Returns:
        bool: True if calculator should be used
...
def use_calculator(input: str) -> str:
    ...
    """
    try:
        # split on << to get the expression and the ending >>
        expr = input.split("<<")[1]
        # return the question + safe_eval on the expression, use [:-2] to take out the ending >>
        return input + str(safe_eval(expr[:-2])) if can_use_calculator(expr) else input
    except:
