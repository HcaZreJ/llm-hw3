...
class EncoderModel(nn.Module):
    ...
    def pooling(self, last_hidden_state, attention_mask):
        ...
        """ 
        # For each batch, find the index of the last valid token
        last_token_indices = torch.sum(attention_mask, dim=1) - 1 # i.e., sum along seq_len dim, then minus 1 because idx start at 0
        
        # Create a long tensor going from 0 to batch_size for selection
        batch_indices = torch.arange(0, last_hidden_state.size(0))
         
        # Use index to get elements, need batch indices so torch doesn't interpret last_token_indices as the seq pos to take for every batch
        last_tokens = last_hidden_state[batch_indices, last_token_indices, :]
                
        # Normalize on L2 norm
        return F.normalize(last_tokens, p=2, dim=-1)
    ...
    def compute_similarity(self, q_reps, p_reps, temperature):
        ...
        - p_reps:  tensor of shape (n_passages, hideen_dim)
        - temperature:  float

        Returns:
        - similarity_matrix: tensor of shape (n_queries, n_passages)
        """        
        return torch.matmul(q_reps, p_reps.T) / temperature
